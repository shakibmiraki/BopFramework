...
export const TransactionsFilterStatus = () => {
  const { values, setFieldValue } = useFormikContext();
  const filter_status_fieldname = "filter_status";
  const isActive = (item) => {
    return values[filter_status_fieldname] === item.id;
  };
    ...
    <div css={css(styles)}>
      <section className="bg-white shadow-sm pt-2 pb-2 mt-3">
        <div className="col-12 p-3">
          <Paragraph text="وضعیت تراکنش های خود را انتخاب کنید"></Paragraph>
        </div>
        {transactionService.fetchStatusFilters()?.map(function (item, i) {
          return (
            <div
              className="col-12 item font-small-1 font-weight-bold"
              key={item.id}
              onClick={() => {
                setFieldValue(filter_status_fieldname, item.id);
                history.goBack();
              }}
            >
