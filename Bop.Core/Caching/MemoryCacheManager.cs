...
namespace Bop.Core.Caching
{
    ...
    public partial class MemoryCacheManager : ILocker, IStaticCacheManager
    {
        ...
        public T Get<T>(string key, Func<T> acquire, int? cacheTime = null)
        {
            ...
            return _provider.Get(key, acquire, TimeSpan.FromMinutes(cacheTime ?? BopCachingDefaults.CacheTime))
                .Value;
        }
        ...
        public async Task<T> GetAsync<T>(string key, Func<Task<T>> acquire, int? cacheTime = null)
        {
            ...
            var t = await _provider.GetAsync(key, acquire, TimeSpan.FromMinutes(cacheTime ?? BopCachingDefaults.CacheTime));
            return t.Value;
        }
